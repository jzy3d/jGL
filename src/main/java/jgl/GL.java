...
//	    case GL_AUTO_NORMAL:
  ...
  private int translate_id(int n, int type, byte lists[]) {
    ...
    case GL_4_BYTES:
      return (int) ((lists[n << 2]) << 24 | (lists[(n << 2) | 1]) << 16 | (lists[(n << 2) | 2]) << 8
          | (lists[(n << 2) | 3]));
    case GL_FLOAT:
      return (int) ((lists[n << 2]) * 16777216.0f + (lists[(n << 2) | 1]) * 65536.0f + (lists[(n << 2) | 2]) * 256.0f
          + (lists[(n << 2) | 3]));
    default:
      return 0;
    }
  ...
  public void glClearColor(float red, float green, float blue, float alpha) {
    ...
    CC.gl_clear_color(gl_util.CLAMP(red, 0.0f, 1.0f), gl_util.CLAMP(green, 0.0f, 1.0f), gl_util.CLAMP(blue, 0.0f, 1.0f),
...
//	    CC.gl_clear_accum_buffer ();
  ...
  public void glLoadMatrixd(double m[]) {
    ...
    float temp[] = { (float) m[0], (float) m[1], (float) m[2], (float) m[3], (float) m[4], (float) m[5], (float) m[6],
        (float) m[7], (float) m[8], (float) m[9], (float) m[10], (float) m[11], (float) m[12], (float) m[13],
        (float) m[14], (float) m[15] };
...
//			      int format, int type, byte pixels [][][]) {
  ...
  public void glTexSubImage2D(int target, int level, int xoffset, int yoffset, int width, int height, int format,
    ...
    CC.gl_tex_sub_image_2d(target, level, xoffset + b, yoffset + b, width + xoffset + b, height + yoffset + b, format,
        size, pixels);
  }
...
//	    CC.gl_map_2 (target, (float)u1, (float)u2, ustride, uorder,
      Context.gl_map_2(target, (float) u1, (float) u2, ustride, uorder, (float) v1, (float) v2, vstride, vorder, np);
    }
    if (ustride < dim) {
...
//	System.out.println ("call draw image");
  ...
  /*
   * public void glJViewport (int x, int y, int width, int height) { glViewport
   * (x, y, width, height); JavaImageSource = new MemoryImageSource
   * (Context.Viewport.Width, Context.Viewport.Height, Context.ColorBuffer.Buffer,
   * 0, Context.Viewport.Width); JavaImageSource.setAnimated (true);
   * JavaImageSource.setFullBufferUpdates (true); JavaImage =
   * JavaComponent.createImage (JavaImageSource); }
   * 
   * public void glJFlush () { if (Context.RenderMode != GL_RENDER) { return; }
